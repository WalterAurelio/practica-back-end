...
const handleRefreshToken = async (req, res) => {
  ...
  console.log('Soy la cookie vencida? ', JSON.stringify(cookies));

  if (!cookies?.jwt) return res.status(401).json({ message: 'No se recibieron refresh tokens en la petición.' });
  ...
  if (!foundUser) {
    jwt.verify(
      ...
      async (err, decoded) => {
        if (err) return res.status(401).json({ message: 'Token vencido que no pertenece a un usuario.' });
    ...
    return res.status(401).json({ message: 'Usuario podría estar siendo hackeado.' })
  }
  ...
  jwt.verify(
    ...
    async (err, decoded) => {
      ...
      if (err || foundUser.username !== decoded.username) return res.status(401).json({ message: 'Refresh token vencido!' });
...
const handleRefreshToken_2 = async (req, res) => {
  const cookies = req.cookies;
